 ...
 * @returns {number}
 */
function getVectorDistance(list) {
	return list.reduce((t, v) => Math.sqrt(t * t + v * v), 0);
}
...
/**
 * Get between two vectors using the largest item difference as distance
 * @param {Array} origin
 * @param {Array} target
 * @param {number} distance
 * @param {function?} easing
...
function fadeVectorByDistanceLazy(origin, target, distance, easing = v => v) {
	const lazyDistance = origin
		.map((v, i) => Math.abs(v - target[i]))
