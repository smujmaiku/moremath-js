...
describe('utils/poly', () => {
	...
	describe('rayTracePolyClosest', () => {
		it('should calculate the closest point of interection', () => {
			const poly = [[0, 0], [2, 0], [1, 1], [0, 1]];
			expect(rayTracePolyClosest(poly, [1, 0])).toEqual([1, 0]);
			expect(rayTracePolyClosest(poly, [-1, 0])).toEqual([0, 0]);
			expect(roundAll(rayTracePolyClosest(poly, [1.5, 2], [0, -1]), -10)).toEqual([1.5, 0.5]);
			expect(roundAll(rayTracePolyClosest(poly, [1.5, 2], [-1, -1]), -10)).toEqual([0.5, 1]);
		});
	});
	...
	describe('isInsidePoly', () => {
		...
		it('should disable checkerboarding', () => {
			const poly = [
				[1, 0], [2, 0], [2, 2], [0, 2],
				[0, 1], [3, 1], [3, 3], [1, 3],
			];
			expect(isInsidePoly(poly, [1.5, 1.5])).toEqual(false);
			expect(isInsidePoly(poly, [1.5, 1.5], true)).toEqual(true);
			expect(isInsidePoly(poly, [1.5, 0.5])).toEqual(true);
			expect(isInsidePoly(poly, [1.5, 0.5], true)).toEqual(true);
			expect(isInsidePoly(poly, [0.5, 0.5])).toEqual(false);
			expect(isInsidePoly(poly, [0.5, 0.5], true)).toEqual(false);
			expect(isInsidePoly(poly, [1, 0])).toEqual(true);
			expect(isInsidePoly(poly, [1, 0], true)).toEqual(true);
			expect(isInsidePoly(poly, [1, -1])).toEqual(false);
			expect(isInsidePoly(poly, [1, -1], true)).toEqual(false);
		});
	});
});
